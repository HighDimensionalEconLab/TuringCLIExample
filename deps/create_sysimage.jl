...
# julia ./deps/create_sysimage.jl --only_package --precompile_tests   #change to only use the one package and not precompile tests, etc.

all_packages = !("--only_package" in ARGS)
precompile_tests = ("--precompile_tests" in ARGS)
...
@info "Now building a custom sysimage for the environment '$project'."
